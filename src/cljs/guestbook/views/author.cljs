...
(def author-controllers
  [{:parameters {:path [:user]}
    :start (fn [{{:keys [user]} :path}] (rf/dispatch [:messages/load-by-author user]))}
   {:parameters {:path [:user]}
    :start (fn [{{:keys [user]} :path}] (rf/dispatch [::fetch-author user]))
    ...
    (fn [{{{:keys [user]} :path} :parameters}]
      (if @(rf/subscribe [::loading?])
        ...
        (let [{{:keys [display-name banner bio]} :profile} @author]
          [:div.content
           [banner-component (or banner "/img/banner-default.jpg")]
           [title]
           (when bio
             [:p bio])
