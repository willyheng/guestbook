                              ...
                              (fn [{r :response}]
                                ...
                                (reset!
                                 ...
                                 (case (:error r)
                                   ...
                                   ;; else
                                   {:server "Unknown Server Error. Please try again!"})))}))))]
      (fn []
        [:<>
         [:h3 "Change Password"]
        ...
        {:disabled (or (:loading @status) (empty? @login) (empty? @password))
         ...
         (fn [_]
           (if (not= @login username)
             (reset! status {:error (str "Login must match current user: " username)})
             (do
                          ...
                          {:params @fields
                           ...
                           (fn [{r :response}]
                             (if (= (:error r) :incorrect-password)
                               (reset! status {:error "Incorrect password, please try again."})
                               (reset! status {:error "Unknown Error Occurred."})))}))))}
        "Delete Account"]]]]))
